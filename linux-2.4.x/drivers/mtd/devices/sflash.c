...
struct sflash_mtd {
	...
	struct mtd_info mtd;
	struct mtd_erase_region_info regions[SFLASH_MAX_BANKS];
};
...
sflash_mtd_init(void)
{
	...
	for (bank = 0; bank < SFLASH_MAX_BANKS; bank++) {
		sflash.regions[i].offset = info->banks[bank].offset;
		sflash.regions[i].erasesize = info->banks[bank].erasesize;
		sflash.regions[i].numblocks = info->banks[bank].numblocks;
