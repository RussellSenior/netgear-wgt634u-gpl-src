...
/* for post a client request */

#define JAVASCRIPT "javascript"
#define JAVATYPE "text/javascript"
#define FORMNAME "formname"
#define ACTION   "ptimeout.cgi"
#define METHOD   "POST"
#define INPUTNAME "inputname"
#define INPUTTYPE "hidden"
#define REL  "stylesheet" 
#define HREF "/form.css"
#define FONT  "Courier"
#define FONTSIZE "10pt" 
...
/* R_UNAUTHORIZED for timeout : 401 */
void send_r_unauthorizedtimeout(request * req, char *realm_name)
{
    SQUASH_KA(req);
    req->response_status = R_UNAUTHORIZED;
    if (!req->simple) {
    ...
    if (req->method != M_HEAD) {
        req_write(req,"<html><head><meta http-equiv='Pragma' content='no-cache'><meta http-equiv='Cache-Control' content='no-cache'><title> Authorization warning</title>\n");
        req_write(req,"<script language=" JAVASCRIPT " type=" JAVATYPE ">function cancelevent(){document.formname.submit();}</script>");
        req_write(req,"</head><body onload=cancelevent()><form name=" FORMNAME " action=" ACTION " method=" METHOD ">");
        req_write(req,"<input type="INPUTTYPE" name=" INPUTNAME " value=""></form></body></html>\n");
        }
    req_flush(req);
}

/* for timeout  response to client  */
void send_r_unauthorizedtoclient(request * req)
{
    SQUASH_KA(req);
    req->response_status = R_REQUEST_OK;
        req_write(req,
                  "<HTML><HEAD><TITLE>Unauthorization warning</TITLE>"
		  "<link rel= " REL " href= " HREF ">"
                  "<STYLE TYPE=" JAVATYPE ">classes.num.all.fontFamily = " FONT ";classes.num.all.fontSize = " FONTSIZE ";</style></HEAD>\n"
                  "<BODY><H1>Unauthorization Warning </H1>\n Please login in again . </BODY></HTML>\n");
        req_flush(req);
}

/* for one user login in */
void send_r_unauthorizedlogin(request * req)
{
    SQUASH_KA(req);
    req->response_status = R_REQUEST_OK;
        req_write(req,
