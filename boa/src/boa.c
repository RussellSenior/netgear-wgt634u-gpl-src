...
struct timeval req_noblocktimeout;
...
int main(int argc, char **argv)
{
    ...
    req_noblocktimeout.tv_sec=180;
    req_noblocktimeout.tv_usec=0l;
    while (1) {
        if (sighup_flag)
	    sighup_run();
        ...
        if (!request_ready) {
            a = select(OPEN_MAX, &block_read_fdset,
		...
		case 0:
		{
			FD_SET(server_s, &block_read_fdset);
			timeoutflag=1;
			req_noblocktimeout.tv_sec=180;
			req_noblocktimeout.tv_usec=0l;
			break;
