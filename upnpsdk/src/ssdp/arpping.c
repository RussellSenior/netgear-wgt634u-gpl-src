...
void mkArpMsg(int opcode, u_long tInaddr, u_char * tHaddr, u_long sInaddr,
	      u_char * sHaddr, struct arpMsg *msg);
...
int arpCheck(u_long inaddr, struct ifinfo *ifbuf, long timeout)
{
    ...
    rv = 1;
    if (openRawSocket(&s, ETH_P_ARP) < 0) {
	return -1;
    }
    ...
    if ((rv = sendto(s, &arp, sizeof(arp), 0, &addr, sizeof(addr))) < 0) {
	syslog(LOG_ERR,"arpCheck: sendto failed with error code %d and errno: %d\n",rv, errno);
	return rv;
    }
	...
	if (select(s + 1, &fdset, (fd_set *) NULL, (fd_set *) NULL, &tm) <
	    0) {
	    syslog(LOG_ERR, "Error on ARPING request: %s",
		  sys_errlist[errno]);
	    if (errno != EINTR) {
		rv = 0;
		syslog(LOG_INFO, "arpCheck: errno != EINTR\n");

	    }
	} else if (FD_ISSET(s, &fdset)) {
	    if (recv(s, &arp, sizeof(arp), 0) < 0) {
		rv = 0;
		syslog(LOG_INFO, "arpCheck: recv returned < 0...!\n");
	    }
	    if (arp.operation == htons(ARPOP_REPLY) &&
		bcmp(arp.tHaddr, ifbuf->haddr, 6) == 0 &&
		*((u_int *) arp.sInaddr) == inaddr) {
		syslog(LOG_INFO,
		      "Valid arp reply receved for this address");
		rv = 0;
		break;
	    }
	}
    ...
    close(s);
    //fprintf(stderr, "arpCheck: exit rv = %d\n", rv);
    syslog(LOG_INFO, "%salid arp replies for this address",
	  rv ? "No v" : "V");
    return rv;
}
	      ...
	      u_long sInaddr, u_char * sHaddr, struct arpMsg *msg)
{
    ...
    memcpy(msg->sInaddr, &sInaddr, 4);  /* source IP address */ 
    bcopy(sHaddr, msg->sHaddr, 6);	/* source hardware address */
    memcpy(msg->tInaddr, &tInaddr, 4);    /* target IP address */
...
int openRawSocket(int *s, u_short type)
{
	...
	syslog(LOG_ERR, "Could not open raw socket");       
        //aplog(EV_DHCPS_SOCK_FAILED, "", NULL);
        return -1;
    }

    if (setsockopt(*s, SOL_SOCKET, SO_BROADCAST, &optval, sizeof(optval))
	== -1) {
	syslog(LOG_ERR, "Could not setsocketopt on raw socket");
        //aplog(EV_DHCPS_SOCK_FAILED, "", NULL);
	return -1;
    }
    return 0;
