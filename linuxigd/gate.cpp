...
char *GateServiceId[] = {"urn:upnp-org:serviceId:Layer3Forwarding"
                        ,"urn:upnp-org:serviceId:WANCommonIFC1"
                        ,"urn:upnp-org:serviceId:WANIPConn1"};

extern char lan_ip_address [16];
extern unsigned char lan_mac_address[6];
                         ...
                         void *Cookie)
{
	switch ( EventType)
	{
		...
		case UPNP_CONTROL_ACTION_REQUEST:
			syslog(LOG_DEBUG,__FILE__ " DeviceCallbackEventHandler:Entered Function\n");
...
{	
	...
	if (UpnpDownloadXmlDoc(DescDocURL, &gateDescDoc) != UPNP_E_SUCCESS)
	{
		syslog(LOG_ERR, "DeviceStateTableInit -- Error Parsing %s\n", DescDocURL);
		ret =UPNP_E_INVALID_DESC;
	}

	gate_udn = SampleUtil_GetFirstDocumentItem(gateDescDoc, "UDN");
	return (ret);
}
...
int Gate::GateDeviceHandleSubscriptionRequest (struct Upnp_Subscription_Request *sr_event)
{
	...
	if ( SampleUtil_CheckServiceUdn( gateDescDoc,sr_event->UDN ,sr_event->ServiceId))
	{
		if (strcmp(sr_event->ServiceId, GateServiceId[GATE_SERVICE_LAYER3FORWARDING]) == 0)
		{
			UpnpAddToPropertySet(&PropSet, "DefaultConnectionService","WANIPConn1");
...
int Gate::GateDeviceHandleActionRequest(struct Upnp_Action_Request *ca_event)
{
	...
	pthread_mutex_lock(&DevMutex);
	if ( SampleUtil_CheckServiceUdn( gateDescDoc,ca_event->DevUDN ,ca_event->ServiceID))
	{
		if (strcmp(ca_event->ServiceID,"urn:upnp-org:serviceId:WANIPConn1") ==0)
		{
		               syslog(LOG_DEBUG,__FILE__ " DeviceHandleAction -- Action Name: %s\n", ca_event->ActionName);
		...
		else if (strcmp(ca_event->ServiceID,GateServiceId[GATE_SERVICE_LAYER3FORWARDING]) == 0)
		{
			if (strcmp(ca_event->ActionName,"GetDefaultConnectionService") == 0)
				result = GateDeviceGetDefaultConnectionService(ca_event);
			else if	 (strcmp(ca_event->ActionName,"SetDefaultConnectionService") == 0)
				result = GateDeviceSetDefaultConnectionService(ca_event);
			else result = 0;
		}
	}
...
int Gate::GateDeviceSetDefaultConnectionService(struct Upnp_Action_Request *ca_event)
{
	return(UPNP_E_SUCCESS);
}
int Gate::GateDeviceGetDefaultConnectionService(struct Upnp_Action_Request *ca_event)
{
	char cDefConSev[256];
	char result_str[512];
	char uidStr[42];
	char macAddressStr[13];
	strcpy(uidStr,"uuid:14b57a40-1dd2-11b2-85af-");
	...
	strcat(uidStr,macAddressStr);
	
	// Convert the response string
	sprintf( cDefConSev, "%s:%s,%s",uidStr ,"WANConnectionDevice:1", "urn:upnp-org:serviceId:WANIPConn1");
	sprintf( result_str,"<u:%sResponse xmlns:u=\"%s\">" "<%s>%s</%s>""</u:%sResponse>",ca_event->ActionName,"urn:schemas-upnp-org:service:WANIPConnection:1","NewDefaultConnectionService", cDefConSev,"NewDefaultConnectionService", ca_event->ActionName);
...
int Gate::GateDeviceAddPortMapping(struct Upnp_Action_Request *ca_event)
{
	...
	{
		syslog( LOG_DEBUG, __FILE__ " *** Hello parameters proto:%s ext_port:%s int_port:%s int_ip:%s enabled:%s\n", proto, ext_port, int_port, int_ip, enabled); 
		prt = getProtoNum(proto);
		if ((prt == 6) || (prt ==17) )
		{
			// Change IP broadcast to lan local broadcast
			if ((prt==17)&&(int_ip!=NULL)&&(strcmp(int_ip,"255.255.255.255")==0))
			{
				strcpy(lan_bcast_ip,lan_ip_address);
				tmp_str=strrchr(lan_bcast_ip,'.');
				if (tmp_str!=NULL)
				{
					strcpy(tmp_str,".255");
					strcpy(int_ip,lan_bcast_ip);
				}
			}
			if (chkIPADDRstring(int_ip)!=0)
			{
				result=m_list.PortMapAdd(NULL, proto, address, atoi(ext_port), int_ip, atoi(int_port), atoi(enabled), desc, 0);
				if (result==1)
				{
