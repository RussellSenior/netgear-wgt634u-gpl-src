...
void *dhcpStart()
{
  ...
  setsockopt(dhcpSocket, SOL_SOCKET, SO_REUSEADDR, (void *) &sock_opt,
          sizeof(sock_opt));
  ...
  if ( setsockopt(udpFooSocket,SOL_SOCKET,SO_BROADCAST,&o,sizeof(o)) )
    syslog(LOG_ERR,"dhcpStart: setsockopt: %m\n");
  o = 1;
  if (setsockopt(udpFooSocket,SOL_SOCKET,SO_REUSEADDR,&o,sizeof(o)))          
  {
         syslog(LOG_ERR,"dhcpStart: setsockopt: %m\n");
         exit(1);
  }
...
void *dhcpInit()
{
  ...
  {
      if ( DebugFlag )
          syslog(LOG_DEBUG,"timed out waiting for a valid DHCP server response\n");
      alarm(TimeOut);
      return &dhcpInit;
  }
