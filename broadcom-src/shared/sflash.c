...
static void
sflash_select(chipcregs_t *cc, uint bank)
{
	uint32 val;
...
static struct sflash_bank *
sflash_bank(chipcregs_t *cc, uint offset)
{
	uint bank;
...
sflash_init(chipcregs_t *cc)
{
	...
	for (bank = 0; bank < SFLASH_MAX_BANKS; bank++) {
		...
		sflash.banks[bank].size += sflash.banks[bank].erasesize * sflash.banks[bank].numblocks;
