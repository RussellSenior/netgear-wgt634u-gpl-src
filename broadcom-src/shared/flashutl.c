...
int FlashEraseRange(unsigned short* dst, unsigned int numbytes);
...
{
	/* Copy the last sector into the scratchpad area */
	unsigned long *val=0, base, nv, *ptr, *src, *end, i;
                           
	nv = flashutl_desc->size - NVRAM_SPACE;
	base = flash_block_base(nv);

	val = ptr = (unsigned long*)malloc( (unsigned long)nv - (unsigned long)base + NVRAM_SPACE );
    
	ASSERT(ptr);
    
	/* bakup the sector's data minus the last NVRAM_SPACE */
	src = (unsigned long*)(FLASH_BASE + base);
	end = (unsigned long*)(FLASH_BASE + nv);
	while (src < end)
		*ptr++ = *src++;

	/* copy in the nvram data */
	src = (unsigned long*)data;
	for(i=0; i<len; i+=4)
		*ptr++ = *src++;

	/* Write the new env. data plus any saved data in the last segment */
	src = (unsigned long*)val;
	ptr = (unsigned long*)(FLASH_BASE + base);
	len = (nv - base) + len;
	FlashWriteRange((unsigned short*)ptr, (unsigned short*)src, len);

	free(val);
}

int
nvRead(unsigned long adr)
{
	unsigned short Data;
    
	Data = *((unsigned char *)(adr));

	return (int)Data;
}

int
FlashEraseRange(unsigned short* dst, unsigned int numbytes)
{
	unsigned long addr = (unsigned long)dst - (unsigned long)FLASH_BASE;
	unsigned long end = addr + numbytes;
	int err = 0;
	...
	while (addr < end) {
		...
		if (err)
			break;
		addr = flash_block_lim(addr);
	}

	if (err)
		DPRINT(("Block erase at 0x%x failed\n", (unsigned int)addr+FLASH_BASE));
	else
		DPRINT(("Done\n"));
...
int
FlashWriteRange(unsigned short* dst, unsigned short* src, unsigned int numbytes)
{
	unsigned long off = (unsigned long)dst - (unsigned long)FLASH_BASE;
	int err;
	
	if (!FlashEraseRange(dst, numbytes)) 
		return 0;
	
	DPRINT(("Writing 0x%x bytes to flash @0x%x ...\n", (unsigned int)numbytes, (unsigned int)dst));
